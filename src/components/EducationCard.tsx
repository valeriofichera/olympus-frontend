...
export function CompactRebases({ quantity, isQuantityExact, asset }: EducationGraphicProps) {
  return (
    <CompactGraphic
      ...
      subtext={t`Receives rebases from ${isQuantityExact ? "" : "≈ "}${quantity} ${asset}`}
    />
  );
}
...
export function LargeWallet() {
  return (
    <LargeGraphic
      ...
      subtext={t`Olympus Give is a means of directing the rebases accrued by your sOHM or sOHM equivalent value of gOHM to another wallet. The first step is depositing your sOHM and specifying a recipient.`}
    />
  );
}
